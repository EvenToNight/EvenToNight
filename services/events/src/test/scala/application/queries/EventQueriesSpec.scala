...
class EventQueriesSpec extends AnyFlatSpec with Matchers:
  ...
  "GetEventQuery" should "return an event if it exists" in {
    ...
    val event = Event.create(
      ...
      status = EventStatus.PUBLISHED, creatorId = OrganizationId.unsafe("org-1"), collaboratorIds = None
    )
    repo.save(event)
  ...
  "GetAllEventsQuery" should "return all published events" in {
    val repo = new FakeEventRepository()
    val event1 = Event.create(None, None, None, None, None, None, EventStatus.PUBLISHED, OrganizationId.unsafe("org-1"), None)
    val event2 = Event.create(None, None, None, None, None, None, EventStatus.DRAFT, OrganizationId.unsafe("org-1"), None)
    repo.save(event1)
    repo.save(event2)
  ...
  "GetFilteredEventsQuery" should "filter events correctly" in {
    ...
    val command = GetFilteredEventsCommand(
      limit = Some(10), offset = Some(0), status = Some(List(EventStatus.PUBLISHED)),
      title = Some("Title"), tags = None, startDate = None, endDate = None,
      organizationId = None, city = None, location_name = None, sortBy = None,
      sortOrder = None, query = None, near = None, other = None, price = None
    )
