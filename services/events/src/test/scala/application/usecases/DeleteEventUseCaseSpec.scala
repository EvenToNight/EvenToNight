...
class DeleteEventUseCaseSpec extends AnyFlatSpec with Matchers:
  ...
  it should "soft delete (cancel) a PUBLISHED event instead of hard deleting it" in {
    ...
    val draft = Event.create(
      title = Some(EventTitle.unsafe("Pub Event")), description = None, poster = None, tags = None, 
      location = Location(city = Some("Rome")).toOption, date = Some(LocalDateTime.now().plusDays(1)),
      status = EventStatus.DRAFT, creatorId = OrganizationId.unsafe("org-1"), collaboratorIds = None
    )
