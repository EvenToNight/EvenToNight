object DockerCommands {
    ...
    const val SETUP_APPLICATION_ENVIRONMENT = "sed -i '' '/^STRIPE_WEBHOOK_SECRET=/d' .env && ./services/payments/scripts/local-webooks.sh & echo '⏳ Waiting for webhook listener...' && until grep -q 'STRIPE_WEBHOOK_SECRET=whsec_' .env 2>/dev/null; do sleep 1; done && echo '✅ Webhook listener ready' && ./scripts/composeApplication.sh --dev up -d --build --force-recreate --wait"
