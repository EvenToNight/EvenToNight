...
\begin{itemize}
  \item \textbf{OpenAPI specs} (REST): \url{https://eventonight.github.io/EvenToNight/openAPI/}
  \item \textbf{AsyncAPI specs} (RabbitMQ): \url{https://eventonight.github.io/EvenToNight/asyncAPI/}
\end{itemize}
...
\paragraph{Database querying.} All application services use \textbf{MongoDB} as their document store. The \texttt{user} and \texttt{event} services (Scala) query MongoDB via the raw Java driver (\texttt{mongodb-driver-sync}). The \texttt{chat}, \texttt{interaction}, \texttt{notification} and \texttt{ticketing} services (TypeScript) use \textbf{Mongoose} as the ODM, with schema decorators and typed model queries. The \texttt{media} service stores binary files in \textbf{MinIO}, an S3-compatible self-hosted object store.

\paragraph{Authentication.} Authentication is handled by \textbf{Keycloak}. On login, Keycloak issues a short-lived JWT access token and a longer-lived refresh token. Every protected API endpoint requires an \texttt{Authorization: Bearer <token>} header. Each service validates the JWT independently using Keycloak's RSA public key; the \texttt{user} service exposes a \texttt{/public-keys} endpoint that the other services fetch at startup. The frontend stores tokens in \texttt{sessionStorage}, automatically refreshes the refresh token before expiry and injects the access token into all outgoing requests.

\paragraph{Authorization.} The system uses \textbf{Role-Based Access Control (RBAC)} with two roles: \texttt{member} and \texttt{organization}. Roles are assigned in Keycloak at registration time and embedded in the JWT. Services extract the role from the token and apply authorization logic at the handler level.
...
\begin{lstlisting}[caption={Domain event envelope},label={lst:event-envelope}]
{
...
\paragraph{Socket.IO: authenticated connections and scaling.} WebSocket connections to the \texttt{notification} service are authenticated: the client sends its JWT and \texttt{userId} during the Socket.IO handshake and the server validates the token before allowing the connection. Socket.IO was chosen also because it natively supports a Redis pub/sub adapter, which would allow the \texttt{notification} service to scale horizontally while still delivering pushes to the correct client regardless of which replica it is connected to.

\paragraph{Key implementation patterns.}
\begin{itemize}
  \item \textbf{Transactional outbox}: each service writes domain events to an outbox collection atomically with the state change (within a MongoDB transaction); a relay process forwards them to RabbitMQ, retrying with exponential backoff until delivery succeeds.

  \item \textbf{Saga pattern}: the ticket purchase flow is implemented as a two-phase saga. Phase 1 reserves inventory and creates an order atomically in MongoDB (TX1). Phase 2 calls Stripe to create a checkout session. If Stripe fails, a compensating transaction releases the reserved tickets and cancels the order. If the service crashes between the two phases, the compensation does not run, tickets remain in \texttt{PENDING} state indefinitely; a scheduled cleanup job that releases orders stuck in \texttt{PENDING} beyond a timeout threshold would be the correct mitigation and is a known limitation of the current implementation.
\end{itemize}
...
Some services are structured around \textbf{Clean Architecture} with Domain-Driven Design principles. The codebase is organised into four layers with strict inward dependency rules:
...
Within this structure, the \texttt{event} and \texttt{notification} services use \textbf{CQRS}, with write operations represented as commands and read operations as queries, each handled by a dedicated class.
