...
\paragraph{Types of users supported by the system:}
\begin{itemize}
...
\paragraph{Functional requirements for all users:}
\begin{itemize}
...
\paragraph{Functional requirements for registered users:}
\begin{itemize}
  ...
  \item Receive notifications about: new followers, new events published by followed organizations, new messages.
\end{itemize}

\paragraph{Functional requirements for users registered as an organization:}
\begin{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Fault Tolerance}: the system must tolerate failures of individual services without affecting the overall platform operation.
  \item \textbf{Security}: the system requires user authentication and enforces authorization to access resources. Only a secure hash of each user's password will be stored.
  \item \textbf{Robustness}: the system must handle incorrect inputs, providing consistent error messages without compromising stability.
  ...
  \item \textbf{Extensibility}: the system must support easy customization and addition of new functionalities.
  \item \textbf{Maintainability}: the system must be easy to maintain with respect to addition of new features and updates, supported by well-structured and well-documented code.
  ...
  \item \textbf{Portability}: the system must be responsive and adapt to different screen sizes and devices.
...
This architectural style was chosen because it addresses several non-functional requirements of the system. In particular, the following architectural properties directly contribute to satisfying the non-functional requirements:
...
\begin{itemize}
  ...
  \item \textbf{Temporal decoupling}: events are stored in a persistent event space until consumers are able to process them. This ensures that producer services remain responsive even if consumer services are temporarily unavailable, supporting fault tolerance and availability. In addition, this storage smooths load peaks and supports extensibility and horizontal scalability by allowing multiple consumer instances.
  \item \textbf{Loosely coupled services}: producers publish events without waiting for consumers to respond and consumers process events independently. This independence supports fault tolerance, maintainability and extensibility.
\end{itemize}
