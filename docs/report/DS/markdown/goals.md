# 0 - Introduction
The project consists of the design and development of a distributed digital platform called EvenToNight, aimed at connecting organizations that promote social events with users interested in discovering and participating in them. The platform, accessible at [https://eventonight.site](https://eventonight.site/), provides both public browsing functionalities and advanced features for registered users.

From a technical perspective, EvenToNight has been designed as a distributed system based on a microservices architecture, enabling modularity, scalability, and independent management of application domains. This choice is motivated by the social nature of the platform, which requires handling many concurrent users and seamless engagement across multiple services.

The distributed architecture ensures reliable data persistence, availability, and openness, allowing the platform to be easily extended and adapted to evolving requirements. This project thus illustrates the application of distributed systems principles in a real-world web-based platform.

# 1 - Goals of the project
The main goals of the project are:

1. **Create a digital ecosystem for social events.**
Provide a platform for exploring, discovering, and interacting with social events and other users.
...
2. The purchased ticket appears on the personal page of the user and can be
downloaded.

**Usage scenario: Browsing and saving events in the preferred language as a registered user**

- **Actor**: Registered user.
- **Objective**: Explore and save events through a platform interface set in the preferred language.
- **Main flow**:
1. The user logs in to the EvenToNight platform.
2. The user selects the preferred language in the platform settings.
3. The user navigates through the event catalog, which is displayed in the chosen language.
4. The user saves selected events to their personal list.
- **Outcome**:
...
- All services are containerized and managed via Docker Swarm.
