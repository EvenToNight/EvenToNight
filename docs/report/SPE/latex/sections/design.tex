...
\begin{longtable}{|L{3.6cm}|L{2.4cm}|L{8.7cm}|}
\hline
...
\hline
\textbf{Event} (entity) & Event & A social event scheduled by an organization; has a lifecycle \texttt{DRAFT} $\rightarrow$ \texttt{PUBLISHED} $\rightarrow$ \texttt{(CANCELLED} \textbar{} \texttt{COMPLETED)} \\
\hline
...
\hline
\textbf{Ticket} & Ticketing & A single seat sold to a single attendee; has its own lifecycle \texttt{PENDING\_PAYMENT} $\rightarrow$ \texttt{ACTIVE} $\rightarrow$ \texttt{(USED} \textbar{} \texttt{REFUNDED} \textbar{} \texttt{CANCELLED} \textbar{} \texttt{PAYMENT\_FAILED)} \\
\hline
...
\begin{longtable}{|L{3.6cm}|L{2.3cm}|L{2.7cm}|L{5.7cm}|}
\hline
...
The contract carried on RabbitMQ has three layers: a hierarchical \textbf{routing key} (\texttt{<context>.\allowbreak<aggregate>.\allowbreak<verb-past>}, e.g. \texttt{event.\allowbreak published}, \texttt{payments.\allowbreak order.\allowbreak confirmed}), a common \textbf{envelope} defined in \texttt{libs/ts-common} and a \textbf{payload schema} owned by each publishing context.
